...
class Tax_Type extends Component {
  ...
  getTaxTypeById = (id) =>
    AppUtil.getAPI(`catalogos/impuesto/${id}`, sessionStorage.getItem('token')).then(response => {
  ...
  saveTaxType = (e) => {
    ...
    if (this.validateForm(t)) {
      if (this.state.tax_type.id === 0) {
        AppUtil.postAPI(`catalogos/impuesto`, this.state.tax_type).then(response => {
          if (response) {
        ...
        AppUtil.putAPI(`catalogos/impuesto/${this.state.tax_type.id}`, this.state.tax_type).then(response => {
          if (response) {
            let data = response ? response.data : [];
            if (Number.isInteger(data)) {
  ...
  ActionButtons = (rowData) => {
    return (
      <Row className="m-2">
        <Col lg="12" sm="12">
          <Button variant="info" className="btn-fill btn-rounded" onClick={() => this.getTaxTypeById(rowData.id)}>
        ...
        <Container fluid>
          ...
          <Row>
            <DataTable
              ...
              slots={{ 6: (cellData, rowData) => this.ActionButtons(rowData, cellData) }}
              options={{
                language: {
              ...
              <Modal.Body>
                <Row className="m-2">
                  <Col sm="12" xl="12">
                    <label>{t("name")}</label>
                    <Form.Group>
                      <Form.Control placeholder={t("name")} type="text" onChange={this._saveStateVariable} name="nombre" required maxLength={100} value={this.state.tax_type.nombre} />
                    </Form.Group>
                  </Col>
                </Row>
                <Row className="m-2">
                  <Col sm="12" xl="6">
                    ...
                    <Form.Group>
                      <Form.Control placeholder={t("code")} type="text" onChange={this._saveStateVariable} name="codigo" required maxLength={2} value={this.state.tax_type.codigo} />
                    </Form.Group>
                  </Col>
                  <Col sm="12" xl="6">
