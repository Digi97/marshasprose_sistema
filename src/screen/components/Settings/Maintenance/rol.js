...
class Rol extends Component {
  ...
  getRol = () =>
    AppUtil.getAPI(`catalogos/roles`, sessionStorage.getItem('token')).then(response => {
      let rolList = response ? response.data : [];
      this.setState({ rolList });
    });

  getRolById = (id) =>
    AppUtil.getAPI(`catalogos/roles/${id}`, sessionStorage.getItem('token')).then(response => {
  ...
  saveRol = (e) => {
    ...
    if (this.validateForm(t)) {
      if (this.state.rol.id === 0) {
        ...
        AppUtil.putAPI(`catalogos/roles/${this.state.rol.id}`, this.state.rol).then(response => {
          if (response) {
            let data = response ? response.data : [];
            if (Number.isInteger(data)) {
  ...
  ActionButtons = (rowData) => {
    return (
      <Row className="m-2">
        <Col lg="12" sm="12">
          <Button variant="info" className="btn-fill btn-rounded" onClick={() => this.getRolById(rowData.id)}>
              ...
              <Modal.Body>
                <Row className="m-2">
                  <Col sm="12" xl="12">
                    <label>{t("name")}</label>
                    <Form.Group>
                      <Form.Control placeholder={t("name")} type="text" onChange={this._saveStateVariable} name="descripcion" required maxLength={100} value={this.state.rol.descripcion} />
                    </Form.Group>
                  </Col>
                </Row>
              </Modal.Body>
